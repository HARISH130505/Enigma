...
router.post('/login', async (req, res) => {
    try {
        ...
        const token = jsonwebtoken_1.default.sign({ isAdmin: true, username }, process.env.JWT_SECRET, { expiresIn: '4h' });
...
router.get('/teams', auth_1.authenticateAdmin, async (req, res) => {
    try {
        const { data: teams, error: teamsError } = await supabase_1.default
            .from('teams')
...
router.post('/team/:teamId/unlock', auth_1.authenticateAdmin, async (req, res) => {
    try {
...
router.get('/export', auth_1.authenticateAdmin, async (req, res) => {
    try {
        const { data: teams } = await supabase_1.default
            .from('teams')
            .select('id, team_name');
        const results = await Promise.all((teams || []).map(async (team) => {
            ...
            const { data: attempts } = await supabase_1.default
                .from('attempts')
                .select('is_correct')
                .eq('session_id', session.id);
...
router.get('/dashboard', auth_1.authenticateAdmin, async (req, res) => {
    try {
        const { count: totalTeams } = await supabase_1.default
            .from('teams')
            .select('*', { count: 'exact', head: true });
        const { count: activeSessions } = await supabase_1.default
            .from('game_sessions')
            .select('*', { count: 'exact', head: true })
            .eq('status', 'active');
        const { count: completedSessions } = await supabase_1.default
            .from('game_sessions')
            .select('*', { count: 'exact', head: true })
            .eq('status', 'completed');
        const { data: recentAttempts } = await supabase_1.default
            .from('attempts')
            .select('*')
